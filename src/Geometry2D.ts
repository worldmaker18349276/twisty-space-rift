...
export function glueIncompleteCutInRegion<T, S>(path: Path<CutSource<T, S>>, cut: Path<S>): Path<CutSource<T, S>> {
  ...
  if (!incomplete_start_cut && !incomplete_end_cut) return path;
