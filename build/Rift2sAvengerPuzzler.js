...
(function (Puzzle) {
    ...
    Puzzle.make = make;
    // side = true: left
    function getCircleEdges(puzzle, side) {
        const edge0 = side ?
    ...
    // side = true: left
    function getCenterEdges(puzzle, side) {
        const edge0 = side ?
    ...
    // side = true: left
    function getTwistPiece(puzzle, side) {
    ...
    // side = true: left
    function twist(puzzle, side, forward) {
...
(function (PrincipalPuzzle) {
    ...
    function twist(puzzle, angle, side) {
        if (side && puzzle.state.type !== StateType.RightShifted) {
            puzzle.state = {
                type: StateType.LeftShifted,
                angle: (puzzle.state.type === StateType.LeftShifted ? puzzle.state.angle : 0) + angle,
            };
            return true;
        }
            ...
            puzzle.state = {
                ...
                angle: (puzzle.state.type === StateType.RightShifted ? puzzle.state.angle : 0) + angle,
            };
            return true;
        }
        else {
    ...
    function rotateRift(puzzle, angle) {
        let rift_angle = puzzle.rift_angle + angle;
        rift_angle = Geo.mod1(rift_angle / (Math.PI * 4)) * (Math.PI * 4);
        puzzle.rift_angle = rift_angle;
    }
    PrincipalPuzzle.rotateRift = rotateRift;
    function offsetRift(puzzle, offset) {
        let rift_offset = puzzle.rift_offset + offset;
        rift_offset = Math.max(Math.min(rift_offset, MAX_RIFT_OFFSET), -MAX_RIFT_OFFSET);
        puzzle.rift_offset = rift_offset;
    }
    PrincipalPuzzle.offsetRift = offsetRift;
    function moveRift(puzzle, point) {
        const left_center = [-puzzle.center_x, 0];
        const right_center = [+puzzle.center_x, 0];
        const n = Math.floor((puzzle.rift_angle + Math.PI) / (Math.PI * 2));
        const { offset, angle } = getHyperbolaFromPoint(left_center, right_center, point);
        const rift_angle = Geo.as0to2pi(angle + Math.PI) - Math.PI + n * Math.PI * 2;
        const rift_offset = Math.max(Math.min(offset, MAX_RIFT_OFFSET), -MAX_RIFT_OFFSET);
        puzzle.rift_offset = rift_offset;
        puzzle.rift_angle = rift_angle;
    }
    PrincipalPuzzle.moveRift = moveRift;
    ...
    function calculateShapes(puzzle) {
        ...
        const left_circle = { center: [-puzzle.center_x, 0], radius: puzzle.radius };
        const right_circle = { center: [+puzzle.center_x, 0], radius: puzzle.radius };
...
(function (PrincipalPuzzleWithTexture) {
    function _make(puzzle, f, draw_image, scale = 1) {
        ...
        const auxiliary_edges = new Set();
        {
            ...
            for (const edge of Puzzle.getCenterEdges(puzzle.space, true)) {
                auxiliary_edges.add(edge.aff.edges[0]);
            }
    ...
    function twist(puzzle, angle, side) {
        PrincipalPuzzle.twist(puzzle.puzzle, angle, side);
    }
    PrincipalPuzzleWithTexture.twist = twist;
    function snap(puzzle) {
    ...
    function rotateRift(puzzle, angle) {
        PrincipalPuzzle.rotateRift(puzzle.puzzle, angle);
    }
    PrincipalPuzzleWithTexture.rotateRift = rotateRift;
    function offsetRift(puzzle, offset) {
        PrincipalPuzzle.offsetRift(puzzle.puzzle, offset);
    }
    PrincipalPuzzleWithTexture.offsetRift = offsetRift;
    function moveRift(puzzle, point) {
        PrincipalPuzzle.moveRift(puzzle.puzzle, point);
    }
    PrincipalPuzzleWithTexture.moveRift = moveRift;
})(PrincipalPuzzleWithTexture || (PrincipalPuzzleWithTexture = {}));
;
//# sourceMappingURL=Rift2sAvengerPuzzler.js.map